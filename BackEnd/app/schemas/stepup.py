...
class StepUpBase(BaseModel):
    name: str = Field(..., min_length=1, max_length=100, example="Cozy Home StepUp")
    size: str = Field(..., min_length=1, max_length=20, example="42")
...
class StepUpCreate(StepUpBase):
    class Config:
        json_schema_extra = {
            "example": {"name": "Cozy Home StepUp", "size": "42", "price": 25.99, "quantity": 50}
        }
