...
class CurrentUser(BaseModel):
    ...
    password_hash is intentionally excluded — it must never be cached or logged.
    Endpoints that need to verify the current password must reload the user from DB.
    """
...
async def get_current_user(request: Request, db: AsyncSession = Depends(get_db)) -> CurrentUser:
    ...
    try:
        # Priority: HttpOnly cookie → Authorization header (for Postman / API clients)
